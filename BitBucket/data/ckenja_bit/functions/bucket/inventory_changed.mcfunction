...
#複数のバケツがあった場合は統合する。
data modify storage ckenja_bit: Tmp set from storage ckenja_bit: Stack
##全要素数
...
execute as @e[type=item,tag=!ckenja_bit,tag=!global.ignore,tag=!global.ignore.kill,distance=..2,nbt={Item:{tag:{ckenja:{ckenja_bit:{}}}}}] run ckenja_bit:bucket/drop/general
