##CkenJa's root Ver0.1.0.1 ckj02
#CKenJa's modulesから名前変更！機能も大幅に増加し、完全に依存させる形に
#名前空間はckenjaからckj02に変更。競合はないぞ！しかも更新も考えてあるぜ！
#
###更新履歴
...
###root - 実装完了
#ckj02:rootをminecraft:tickに登録する。
#そしてfunctionをサーバー、プレイヤー、エンティティの3つに分けることで大幅な軽量化(誇張)
#さらにckj02:root/in_root,early,standard,lateを呼び出すことで、遅延読み込み的なことをする。
#最後にckj02:root/reset_scoreを呼び出してスコアリセット！これでスコアボードが大幅に扱いやすくなる！
#Lantern Loadと似たような考えだね。https://github.com/LanternMC/Load
#※in_rootは原則ckj02内でのみ使用

###score - 実装完了
#rootに依存することでスコアボードを扱いやすくなったぜ！
#前作からスコア名は接頭尾がckenjaからckj02になったことで競合はないぞ！しかも更新も考えてあるぜ！
#
####list
#ckj02_sneak スニーク時間。スニークしてない時は0になる。スニーク判定のpredicate付き。
#ckj02_coas carrot on a stickの略。使用判定。
...
###data
#あらかじめデータや値を保存しておくckj02_dataスコアボードに関するモジュール
####list
