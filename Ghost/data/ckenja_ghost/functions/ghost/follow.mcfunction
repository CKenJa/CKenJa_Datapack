...
#プレイヤーの方向を向く
execute if entity @a[tag=ckenja_ghost_master,distance=..3] run teleport @s ~ ~ ~ facing entity @a[tag=ckenja_ghost_master,limit=1]
#離れていたらTP移動
execute if entity @a[tag=ckenja_ghost_master,distance=3..128] run teleport @s ^ ^ ^0.5 facing entity @a[tag=ckenja_ghost_master,limit=1]
