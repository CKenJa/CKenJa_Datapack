...
#aecが遠ければtp
execute if entity @e[distance=1..24,tag=ckenja_ghost_aec] run tp @s ^ ^ ^0.4 facing as @e[tags=ckenja_ghost_aec,sort=nearest,limit=1,distance=1..24]
#一定時間ごとに敵対雪玉
scoreboard players operation #tmp ckj01_data = @s ckenja_ghost2
scoreboard players operation #tmp ckj01_data %= #100 ckj01_data
execute if score #tmp ckj01_ghost matchas 0 as @e[type=#ckenja_ghost:enemy,tag=!global.ignore.kill,tag=!global.ignore,distance=..8] run function ckenja_ghost:ability/snowball
